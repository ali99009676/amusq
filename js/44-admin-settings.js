...
const SET = {
  ...
  GROUPS: [
    ...
    /*
      ★ طريقة الدفع نصٌّ يكتبه المشرف لا شيء نفترضه عنه.
      كلٌّ يقبض بطريقته: تحويل، STC Pay، نقدًا في الكلية. وكتابةُ طريقةٍ
      بعينها في الكود تعني تعديل الكود كلما تغيّر حسابٌ بنكي — وتعني أن
      الطالب يقرأ تعليمات لا تنطبق على أحد.
    */
    { id:'pay', title:'الشراء والتفعيل',
      sub:'ما يراه الطالب حين يفتح مادة مدفوعة لا يملكها. اكتب {السعر} و{المادة} حيث تريد، وتُبدَّلان بقيمتَي المادة التي يفتحها.',
      fields:[
      { k:'pay_note', t:'area',
        label:'كيف يدفع لك الطالب؟ — يظهر له في بطاقة المادة المقفلة',
        ph:'حوّل {السعر} على حساب الراجحي SA00 0000 0000 0000 0000، أو STC Pay على 0500000000، وأرسل الإيصال على واتساب مع اسم المادة «{المادة}» ويصلك رمز التفعيل.' },
      /* ★ الانتظار المعلوم يُحتمل والمجهول يُشتكى منه — والمشرف وحده يعرف
         متى يفتح لوحته، فهو من يكتب الوعد. */
      { k:'review_eta', t:'text', label:'متى تفتح الطلبات؟ — يظهر للطالب بعد «حوّلتُ»' }
    ]},
    /*
      ★ «من سعر البيع» و«صافي الربح» مكتوبتان في التسميتين لا في وثيقة.
      الرقمان يبدوان متبادلين وليسا كذلك: أحدهما يُحسب به المال، والآخر
      يُعرض به للرافع. وخلطُهما يعني إما أن تدفع ضعف ما نويت، أو أن تَعِد
      بنصفٍ وتدفع ربعًا.
    */
    { id:'earn', title:'أرباح الرافعين', sub:'نصيب من يرفع المادة، وحدّ التحويل', fields:[
      { k:'uploader_share_pct', t:'num', label:'نصيب الرافع ٪ من سعر البيع', min:0, max:100 },
      { k:'net_margin_pct',     t:'num', label:'صافي ربحك ٪ من سعر البيع (للعرض فقط)', min:1, max:100 },
      { k:'payout_min_halalas', t:'num', label:'أقل مبلغ للتحويل (بالهللات)', min:100, max:1000000 },
      { k:'payouts_open',       t:'bool', label:'طلبات التحويل مفتوحة' }
    ]}
  ],
...
function setField(f, value, onChange){
  ...
  let inp;
  if (f.t === 'area'){
    // ★ المثال داخل الحقل لا في وثيقة جانبية: حقلٌ فارغ اسمه «كيف يدفع لك
    //   الطالب؟» يترك المشرف يخمّن الصيغة والنائبات معًا.
    inp = el('textarea', { class:'input', rows: f.ph ? '4' : '2', 'data-k':f.k,
      placeholder: f.ph || null });
    inp.value = value || '';
  }
  else if (f.t === 'num'){
