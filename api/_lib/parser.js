...
/*
  ★ الفراغات بين الرموز محدودة العدد لا مفتوحة (\s{0,8} لا \s*).
  ثلاثة \s* متجاورة لا يفصل بينها إلا رموزٌ اختيارية تعني أن سطرًا من
  خمسة آلاف فراغ بلا رقم يُجرَّب بكل تقسيماته قبل أن يفشل — ١١ ثانية
  للسطر الواحد، وعشرون سطرًا تأكل مهلة الخادم كلها (تدقيق H-04).
  الفراغ الأول وحده يبقى مفتوحًا لأن ما بعده محدود، فالتراجع خطّي.
  ثمانية فراغات بين «Q» ورقمه أكثر مما يكتبه أحد.
*/
...
const Q_START = /^\s*(?:Q|س|السؤال|سؤال)?\s{0,8}\.?\s{0,8}([\d٠-٩]{1,4})\s{0,8}[).\-:]\s+/i;

/* بداية خيار: حرف لاتيني A-E أو حرف عربي (أ ب ج د هـ) متبوعًا بقوس أو نقطة أو شرطة */
const OPT_START = /^\s*(هـ|[A-Ea-e]|[أابجده])\s{0,8}[).\-]\s+/;

/* سطر إجابة معلنة — بالعربية أو الإنجليزية، بحرفٍ لاتيني أو عربي */
const ANS_LINE =
  /^\s*(?:ANSWER|ANS|KEY|CORRECT|الاجابة|الإجابة|الجواب|الحل|الصحيح)\s{0,8}(?:الصحيحة?|الصحيح)?\s{0,8}(?:هي|هو)?\s{0,8}[:\-.]?\s{0,8}(هـ|[A-Ea-e]|[أابجده])(?=\s|$|[).,،])/i;
...
*/
const EXPL_LINE = /^\s*(?:EXPLANATION|EXPLAIN|RATIONALE|WHY|NOTE|الشرح|التعليل|التفسير|السبب|ملاحظة)\s{0,8}[:\-.]\s*/i;
